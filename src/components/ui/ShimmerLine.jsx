...
export default function ShimmerLine({ tone = 'light', orientation = 'vertical', thick = false, className = 'h-16' }) {
  ...
  if (orientation === 'horizontal') {
    return (
      <span className={`relative block ${thick ? 'h-[3px]' : 'h-[1.5px]'} overflow-hidden ${track} ${className}`}>
        <motion.span
          aria-hidden="true"
  ...
  return (
    <span className={`relative block ${thick ? 'w-[3px]' : 'w-[1.5px]'} overflow-hidden ${track} ${className}`}>
      <motion.span
        aria-hidden="true"
