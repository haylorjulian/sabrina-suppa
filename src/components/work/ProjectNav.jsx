...
// The active project is marked with the one rare accent (synthetic-flesh) as an
// extended tick — never a fill — per DESIGN.md. Siblings stay dimmed and light on
// hover. Desktop-only; hidden with a single project.
      ...
      {siblings.map((p) =>
        p.active ? (
          <span key={p.href} aria-current="page" className="flex items-center gap-3">
            <span className="font-neue-haas-display text-[15px] font-light italic text-bone-porcelain">
              {p.title}
            </span>
          ...
          <Link key={p.href} href={p.href} className="group flex items-center gap-3">
            <span className="font-neue-haas-display text-[15px] font-light italic text-bone-porcelain/40 transition-colors duration-300 group-hover:text-bone-porcelain/80 motion-reduce:transition-none">
              {p.title}
            </span>
            <span
              aria-hidden="true"
